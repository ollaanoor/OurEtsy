...
dataService
  ...
  .then((data) => {
    ...
    document.getElementById("banner-right-title").textContent =
      selectedBanner.name;
    document.getElementById("banner-right-img").src = selectedBanner.image;

    document
      .getElementById("banner-right")
      .setAttribute("data-category-id", data.categories[randomIndexCat].id);
    document
      .getElementById("banner-right")
      .setAttribute("data-subcategory-id", cat[randomIndexSub].id);
...
fetch("../Resources/JSON/banner.json")
  ...
  .then((data) => {
    ...
    var imgRandomIndex = Math.floor(
      Math.random() * banners[randomIndex].image.length
    );

    // Update the HTML content
    document.getElementById("banner-left-title").textContent =
      selectedBanner.title;
    document.getElementById("banner-left-text").textContent =
      selectedBanner.text;
    document.getElementById("banner-left-img").src =
      selectedBanner.image[imgRandomIndex];
    document.getElementById("banner-left").style.backgroundColor =
      selectedBanner.bgcolor;
    ...
    document
      ...
      .addEventListener("click", function () {
        ...
        var cat = mydata.categories.find(
          (cat) => selectedBanner.category == cat.name
        );
...
window.onload = function () {
  ...
  gridContainer.addEventListener("click", function (event) {
    ...
    if (event.target.closest(".fav-btn button")) {
      ...
      // }
      dataService.toggleFav(
        button,
        item.dataset.categoryId,
        item.dataset.subcategoryId,
...
function renderProducts(data) {
  ...
  for (let i = 0; i < 6; i++) {
    ...
    productCard1.setAttribute(
      "data-subcategory-id",
      products1[randomIndexCat].id
    );
  ...
  for (let i = 0; i < 6; i++) {
    ...
    productCard2.setAttribute(
      "data-category-id",
      data.categories[randomIndexCat].id
    );
  ...
  for (let i = 0; i < 6; i++) {
    // var products3 = data.categories[7].subcategories[4].products;
    var randomIndexCat = Math.floor(Math.random() * data.categories.length);
    var randomIndexSub = Math.floor(
      Math.random() * data.categories[randomIndexCat].subcategories.length
    );
    var products3 =
      data.categories[randomIndexCat].subcategories[randomIndexSub].products;
    ...
    document.getElementById("subcat-name").textContent =
      data.categories[randomIndexCat].subcategories[randomIndexSub].name;
    document.getElementById("cat-title").textContent =
      data.categories[randomIndexCat].title;
